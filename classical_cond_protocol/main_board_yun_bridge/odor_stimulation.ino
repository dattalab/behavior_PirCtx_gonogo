...
void odor_stimulation(int mode, int current_block, int nb_trials, int block_order[], int duration_odor_sampling, int duration_wait, int duration_outcome, int duration_interstimulus_interval, int start_assessment_window, int duration_assessment_window, int nb_odors, int odors[], int odor_valence[], String odor_name[], int default_flows[], int carrier_flows[], int odor_flows[]){
  // Setup the olfactometer
    ...
    Serial.println((String) millis() + F(",BIP,") + String(current_block) + "," + String(nb_trials) + "," + String(nb_odors));
    
    // # Block initiation duration
    // time,BID,block_id,odor_sampling,wait_time,outcome,ISI
    Serial.println((String) millis() + F(",BID,") + String(current_block) + "," + String(duration_odor_sampling) + "," + String(duration_wait) + "," + String(duration_outcome) + "," + String(duration_interstimulus_interval));
    
    // # Block initiation assessment window
    // time,BIW,start_time,duration
    Serial.println((String) millis() + F(",BIW,") + String(current_block) + "," + String(start_assessment_window) + "," + String(duration_assessment_window));
    
    // # Block initiation odor list
    // For each odor:
    // time,BIO,block_id,odor_id,odor_name,odor_valence,valve
    for(int i=0; i<nb_odors; i++){
      Serial.println((String) millis() + F(",BIO,") + current_block + "," + (i+1) + "," + odor_name[i] + "," + odor_valence[i] + "," + odors[i]);
    }
    ...
    for (int trial_id=1; trial_id < (nb_trials + 1); trial_id++) {
      ...
      Serial.println((String) F("// ") + millis() + F(",ITI,") + String(current_block) + "," + String(trial_id) + "," + String(duration_ITI[trial_id-1]));
      delay(duration_ITI[trial_id-1]);
      
      // Send a command to log trial initiation
      // time,O,block_id,trial_id,valve_identity
      Serial.println((String) millis() + F(",O,") + String(current_block) + "," + String(trial_id) + "," + String(block_order[trial_id-1]+1));
        ...
        while(state < 4){
          ...
          if(lickState != lastLickState){ // if status has changed
            if(lickState == HIGH){ // if a lick just started
              ...
              Serial.println((String) millis() + F(",L,") + String(current_block) + "," + String(trial_id) + "," + String(countLicks) + F(",1"));
            }
            else{ // if a lick just ended
              // Log the lick
              // time,L,block_id,trial_id,lick_id,0
              Serial.println((String) millis() + F(",L,") + String(current_block) + "," + String(trial_id) + "," + String(countLicks) + F(",0"));
            }
          ...
          switch(state)
          {
            ...
            case 2:
              if((outcome_on == 0) && (outcome_code != 0)){
                ...
                Serial.println((String) millis() + F(",US,") + String(current_block) + "," + String(trial_id) + "," + String(outcome_code) + F(",1"));
              }
