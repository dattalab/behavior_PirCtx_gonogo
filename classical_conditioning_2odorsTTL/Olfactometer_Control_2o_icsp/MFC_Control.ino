...
void setMFCUnitsToSLPM(char* address) {
   ...
   if (!response.startsWith(String("!"+ String(address)+ ",USLPM"))) {
     Serial.println(F("ERROR: bad MFC reponse to units change."));
     return;
   }
}
...
void changeMCFAddress_core(char* new_address, char* new_address_name); // do not call this directly

void changeMCFAddress_Odor() {changeMCFAddress_core(odor_address,"ODOR");}
void changeMCFAddress_Carrier() {changeMCFAddress_core(carrier_address,"CARRIER");}
...
void changeMCFAddress_core(char* new_address, char* new_address_name) {
